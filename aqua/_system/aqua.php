 ...
 * ---------------------------------------------------
 * 
 * [v2.1.0] 2020.04.22 - 이정훈
 *  - aqua/_config/menu.json 파일을 읽어들여 _aqua_config 변수에 추가한다.
 * 
...
if( is_dir( $_aqua_val['dir_path'] ) ) {
    ...
    while (false !== ($_aqua_val['entry'] = readdir($_aqua_val['open_dir_opj']))) {
        ...
        # 영문 폴더명에 한해서 작업 진행
        preg_match('/([a-z])/i', $_aqua_val['entry'], $_aqua_val['match_result'], PREG_OFFSET_CAPTURE);
        
        # 영문 폴더명인 경우 mathch_result 가 배열로 반환하므로 count 함수로 체크
        if( count( $_aqua_val['match_result'] ) ){
...
# 메뉴 파일 확인
$_aqua_val['menu_dir'] = $_SERVER['DOCUMENT_ROOT']. '/aqua/_config/menu.json';

$_aqua_val['menu_get_file'] = @fopen( $_aqua_val['menu_dir'], "r");

if( $_aqua_val['menu_get_file'] ) {

    $_aqua_val['menu_get_file_content'] = fread( $_aqua_val['menu_get_file'], filesize( $_aqua_val['menu_dir'] ));

    fclose( $_aqua_val['menu_get_file'] );
    $_aqua_val['menu_get_file'] = '';

    $_aqua_config['view']['menu_info'] = json_decode( $_aqua_val['menu_get_file_content'], true );

    // echoBr( $_aqua_config['view']['menu_info'] ); exit;
    
} 

$_aqua = new aqua;

// $log = $_aqua->new('accessLog');
// $log->execute('access');
?>