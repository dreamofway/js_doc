...
/**
 * ---------------------------------------------------
 * AQUA Framework fileUploadHandler  v1.0.0
 * ---------------------------------------------------
 * 설명
 * ---------------------------------------------------
 * 
...
class fileUploadHandler extends aqua {
    ...
    private $uplaod_allow_ext = '/(jpg|jpeg|png|gif|zip|hwp|xlsx|xls|txt|pdf|pptx|doc|apk)/i'; # 업로드 허용파일
...
// echoBr( $this->addFiles );
        ...
        if( count( $this->upload_files_arr ) > 0 ) {
            ...
            foreach( $this->upload_files_arr AS $idx=>$item ) {
                ...
                if(@move_uploaded_file($tmp_name, $upload_file_path )) {
                    ...
                    $this->uploaded_files_arr[ $idx ]['origin_name'] = $item['current_file'];
    ...
    public function dbGetFile( $arg_where ){
        ...
        $query = " SELECT * FROM " . $this->use_table . " WHERE " . $arg_where . ' ORDER BY idx DESC LIMIT 1';        
